...
This section aims to provide an example of the execution of the two algorithms taking the unknown language $U$ defind by $regex(\varepsilon+(b+aa+abb)(a+b)^*)$ % $+(b+aa+abb)(a+b)*
...
\begin{algorithm}[H]Let's start with the regular expression of $U = regex(\varepsilon+(b+aa+abb)(a+b)^*)$. The residuals of this language can be found recursively with \cref{algo:find_residuals}.
  ...
  $E \gets \{\varepsilon\}$\tcp*{The list of words to treat}
  \While{$E.length > 0$}{
    $current \gets E.pop()$\;
